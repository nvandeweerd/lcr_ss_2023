...
BibOptions(check.entries = FALSE,
           ...
           max.names = 3)
bib <- RefManageR::ReadBib(here("slides", "references.bib"), check = TRUE)

```

```{r xaringan-banner, echo=FALSE}
xaringanExtra::use_banner(
  bottom_center = paste0("Last edited: ", Sys.time(), " | ", "Slides available here: github.com/nvandeweerd/lcr_ss_2023"),
  exclude = "title-slide"
)
```
...
`r icons::fontawesome("search")` Research interests:
  ...
  - Crowdsourcing language assessment project 
...
participant_info <- openxlsx::read.xlsx(here("info", "Learner Corpus Research Summer School Registration(1-29).xlsx")) %>%
  select(Affiliation, Status, Lat, Long) %>%
...
```{r map}
leaflet(data = participant_info) %>%
  addTiles() %>%  # Add default OpenStreetMap map tiles
  addMarkers(~Long, ~Lat, label=~as.character(Affiliation))
```
...
participant_info %>%
  mutate(Status = str_replace_all(Status, "PhD student.*", "PhD Student")) %>%
  ggplot(aes(y = Status))+
  geom_bar()+
...
## What are the advantages of automatic annotation?
...
Open `activity_02_pos-tagging.docx` and follow the instructions.

**R Option**

Open `activity_02_pos-tagging.R` and follow the instructions.
...
.pull-right[
.content-box-blue[
`r icons::fontawesome("lightbulb")` Can easily be [converted](https://www.convertcsv.com/json-to-csv.htm) into CSV/Excel format.  
...
Ragheb, M., & Dickinson, M. (2012). Defining syntax for learner language annotation. Coling2012. 965–974.
